...
namespace TableForge.UI
{
    internal class TableSize
    {
        ...
        public Vector2 GetTotalSize(bool useStoredValues)
        {
            ...
            {
                if (_tableAttributes.ColumnHeaderVisibility == TableHeaderVisibility.Hidden && row.Key == string.Empty) 
                    continue;

                string rowId = row.Key == string.Empty && _table.IsSubTable ? _table.ParentCell.Id : row.Key;
                if (transposed)
                {
            ...
            {
                if (_tableAttributes.RowHeaderVisibility == TableHeaderVisibility.Hidden && column.Key == string.Empty)
                    continue;
                
                string columnId = column.Key == string.Empty && _table.IsSubTable ? _table.ParentCell.Id : column.Key;
                if (transposed)
                {
        ...
        public Vector2 GetHeaderSize(CellAnchor cellAnchor)
        {
            ...
            if (!transposed)
            {
                ...
                {
                    Row row => new Vector2(_columnPreferredSizes[string.Empty].x, _rowPreferredSizes[row.Id].y),
                    Column column => new Vector2(_columnPreferredSizes[column.Id].x, _rowPreferredSizes[string.Empty].y),
                    _ => new Vector2(_columnPreferredSizes[string.Empty].x, _rowPreferredSizes[string.Empty].y)
                };
            }
            
            return cellAnchor switch
            {
                Row row => new Vector2(_rowPreferredSizes[row.Id].x, _columnPreferredSizes[string.Empty].y),
                Column column => new Vector2(_rowPreferredSizes[string.Empty].x, _columnPreferredSizes[column.Id].y),
                _ => new Vector2(_rowPreferredSizes[string.Empty].x, _columnPreferredSizes[string.Empty].y)
            };
        }
        ...
        public void AddHeaderSize(CellAnchor cellAnchor, Vector2 size)
        {
            ...
            {
                case Row row:
                    AddColumnSize(string.Empty, row.Id, size);
                    AddRowSize(row.Id, string.Empty, size);
                    break;
                case Column column:
                    AddRowSize(string.Empty, column.Id, size);
                    AddColumnSize(column.Id, string.Empty, size);
                    break;
                default:
                    AddRowSize(string.Empty, string.Empty,size);
                    AddColumnSize(string.Empty,string.Empty, size);
                    break;
            }
        }
        ...
        public void RemoveHeaderSize(CellAnchor cellAnchor)
        {
            ...
            {
                case Row row:
                    RemoveColumnSize(string.Empty, row.Id);
                    RemoveRowSize(row.Id, string.Empty);
                    break;
                case Column column:
                    RemoveRowSize(string.Empty, column.Id);
                    RemoveColumnSize(column.Id, string.Empty);
                    break;
                default:
                    RemoveRowSize(string.Empty, string.Empty);
                    RemoveColumnSize(string.Empty, string.Empty);
                    break;
            }
        }
